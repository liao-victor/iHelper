...
<head>
    ...
    <title>Search for Student Information</title>
</head>
<body bgcolor="#EEEEEE">
<h2>General search of student information</h2>
...
{
    echo <<<END
  <form action="search_stud.php" method="post">
  Search:
...
if(!isset($_REQUEST['first'])) {
    echo "Enter text (Name, ID, Phone or Email)<br>
         You can enter the complete word or an initial substring.<p>";
    get_input();
}
else {
    ...
    if (empty($_REQUEST['first'])) {
        ...
        // display results
        print_results($emp, 'Student Information');
...
function get_employees($conn, $firstname)
{
    ...
    $sql = "BEGIN VIEW_PEOPLE.STUD_GENERAL_SEARCH(:firstname,:refcur); END;";
...
function print_results($returned_records, $report_title)
{
    ...
    echo '<input type="button" name="Go Back" value="Back" onclick="location.href=\'search_stud.php\'">';
}
